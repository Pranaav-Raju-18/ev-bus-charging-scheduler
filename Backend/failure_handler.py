"""Operational failure helpers for planned constraints and runtime charger disruptions."""
...
class FailureHandlerMixin:
    """Convert operational failures into solver constraints and charger availability windows."""

    def _operational_failures(self):
        ...
        if allowed_types is not None:
            ...
            failures = [
                failure for failure in failures if failure.get("type") in allowed_types
            ]

        return failures

    def _all_enabled_operational_failures(self):
        if not self._include_operational_failures():
            return []

        return OPERATIONAL_FAILURES

    def _include_operational_failures(self):
        return OPERATIONAL_FAILURE_SETTINGS.get(
            "include_operational_failures",
            False,
        )

    def _failure_window_minutes(self, failure):
    ...
    def _operational_failures_summary(self):
        if not self._include_operational_failures():
            return {
                "enabled": False,
                "failures": [],
            ...
            failures.append(
                {
                    ...
                    "charging_duration_minutes": failure.get(
    ...
    def _slow_charging_operational_failures(self, station_id):
        failures = [
            failure
            for failure in self._operational_failures()
    ...
    def _normal_charging_time(self):
        return CHARGER_CONFIG["charging_duration_minutes"] + CHARGER_CONFIG.get(
            "charger_setup_duration_minutes", 0
        )

    def _max_charging_time(self):
        slow_durations = [
    ...
    def _charger_unavailable_windows(self):
        ...
        for failure in self._operational_failures():
            ...
            if failure_type == "CHARGER_DOWN":
                ...
                for charger_number in range(
                    ...
                    unavailable_windows[charger_id].append((start_minute, end_minute))
    ...
    def _charger_has_conflict(
        ...
        return any(
            ...
            for unavailable_start, unavailable_end in unavailable_windows.get(
                charger_id, []
            )
        )
"""Operational failure helpers for planned constraints and runtime charger disruptions."""
